# -*- coding: utf-8 -*-
"""
Harshita's Birthday Surprise Page: Optimized & Fully Functional
"""
...
for key, default in [("unlocked", False), ("slideshow_audio_bytes", None), ("slideshow_start_time", None)]:
...
def show_secret_code():
    ...
    landing_image = load_image(IMAGE_PATH)
    if landing_image:
        st.image(landing_image, use_column_width=True)
...
def show_slideshow():
    ...
    audio_bytes = st.session_state.slideshow_audio_bytes
    if audio_bytes is None:
        audio_bytes = load_audio_bytes(SLIDESHOW_SONG_PATH)
        st.session_state.slideshow_audio_bytes = audio_bytes

    if audio_bytes:
        st.audio(audio_bytes, format="audio/mp3", start_time=elapsed)
    else:
...
def show_cake():
    ...
    cake_img = load_image(CAKE_IMAGE_PATH)
    if cake_img:
        st.image(cake_img, use_column_width=True)
    else:
        st.warning("Cake image not found! Make sure HARSHICAKE.png is in the repo.")

    cake_audio = load_audio_bytes(CAKE_SONG_PATH)
    if cake_audio:
        st.audio(cake_audio, format="audio/mp3", start_time=0)
    else:
